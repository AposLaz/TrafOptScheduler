...
if [[ -z "$1" || -z "$2" ]]; then
echo ""
echo "You have not provided a Terraform path or Terraform Variables"
echo "SYNTAX = ./plan.sh <PATH> <PATH>/<TF_VARS>"
echo "EXAMPLE = ./plan.sh terraform/instance terraform/instance/variables.tfvars"
echo ""
exit
fi
...
########################################################################
terraform -chdir="$1" apply -var-file="$2" -auto-approve -json
