 ...
 */
export const PROMETHEUS_NODE_INFO = async (clusters: Array<ClusterTypes>) => {
  /**
   ...
   */
  // await Promise.all(
  //   clusters.map(async (cp, index) => {
  //     //get memory
  //     const memory_nodes = await axios.get(
  //       `http://${cp.cluster_ip}/prometheus/api/v1/query`,
  //       {
  //         params: {
  //           query:
  ...
  //     memory_nodes.data.data.result
  //       .sort((a, b) => a.metric.node.localeCompare(b.metric.node))
  //       .map((nodes_data, i) => {
  ...
  //       .sort((a, b) => a.metric.node.localeCompare(b.metric.node))
  //       .map((nodes_data, i) => {
  //         clusters[index].nodes[i].memory.max_memory = Number(
  //           nodes_data.value[1]
  //         );
  //       });
  //   })
  ...
  //       //memory
  //       node.memory.node_available_memory =
  //         node.memory.max_memory - node.memory.requested_memory;
  //     });
  //   })
  // );
